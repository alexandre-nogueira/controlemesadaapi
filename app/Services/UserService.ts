...
export class UserService {
  ...
  //Check if the parent child relationship
  public async isParent(parentId: number, childId: number) {
    const child = await this.getById(childId);
    return !!(child?.parent_user_id === parentId);
  }
  ...
  public async validateUserType(request: RequestContract) {
    const authSchema = schema.create({
      user_type: schema.number([rules.range(1, 2)]),
    });
    const payload = await request.validate({ schema: authSchema });
