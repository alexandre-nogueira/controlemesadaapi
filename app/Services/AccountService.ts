...
export class AccountService {
  ...
  public async validateAccountType(request: RequestContract) {
    //Create schemma to validate account
    const accountSchema = schema.create({
      acct_type: schema.enum(Object.values(AccountTypes)),
    });
  ...
  public async getMany(userId: number) {
    return await Account.query().where('user_id', userId);
  }
  ...
  private async isAccountOwner() {
    if (this._accountId && this._userId) {
      ...
      if (account) {
        ...
        const user = await userService.getById(account.user_id);
        return (
          account.user_id == this._userId ||
          this._userId == user?.parent_user_id
        );
      }
    }
    return false;
